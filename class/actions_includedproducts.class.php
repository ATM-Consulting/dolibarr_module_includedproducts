...
/**
 * \file    includedproducts/class/actions_includedproducts.class.php
...
class ActionsIncludedproducts
{
	...
	public function pdf_getlinetotalexcltax($parameters, &$object, &$action, $hookmanager) {
		...
		if ((in_array('propalcard', $TContext) || in_array('ordercard', $TContext) || in_array('invoicecard', $TContext))
			&& !empty($is_included_product)) {
			$hookmanager->resPrint = $langs->trans('Included');
			return 1;
		}
	}
						...
						$(document).ready(function () {
							...
							foreach($object->lines as &$line)
							{
								...
								if(empty($line->array_options['options_includedproducts_isincludedproduct']))
								{
									...
									td.append('&nbsp;<button type="button" id="included-products-dialog-button" class="classfortooltip" data-target-model-pdf="<?php echo $object->model_pdf; ?>" data-target-lang-id="<?php echo $object->thirdparty->default_lang ? $object->thirdparty->default_lang : $langs->getDefaultLang(); ?>" data-target-element="<?php echo $object->element; ?>" data-target-id="<?php echo $object->id; ?>" data-target-idline="<?php echo $line->id; ?>" title="<?php echo $langs->trans("OpenIncludedProductBox"); ?>" ><i>C</i></button>');
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		/* print_r($parameters); print_r($object); echo "action: " . $action; */
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2'))) {		// do something only for the context 'somecontext1' or 'somecontext2'
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("IncludedproductsMassAction").'</option>';
		}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'includedproducts') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("Includedproducts");
			$this->results['picto'] = 'includedproducts@includedproducts';
		}
	...
	public function restrictedArea($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['features'] == 'myobject') {
			if ($user->rights->includedproducts->myobject->read) {
	...
	public function completeTabsHead(&$parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['mode'] == 'remove') {
			...
			if (in_array($element, ['context1', 'context2'])) {
				...
				$parameters['head'][$counter][0] = dol_buildpath('/includedproducts/includedproducts_tab.php', 1) . '?id=' . $id . '&amp;module='.$element;
				$parameters['head'][$counter][1] = $langs->trans('IncludedproductsTab');
				if ($datacount > 0) {
					$parameters['head'][$counter][1] .= '<span class="badge marginleftonlyshort">' . $datacount . '</span>';
				}
				$parameters['head'][$counter][2] = 'includedproductsemails';
				$counter++;
			}
