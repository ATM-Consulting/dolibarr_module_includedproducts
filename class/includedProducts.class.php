...
/**
 * Class for IncludedProducts
 */
class IncludedProducts extends CommonObject
{
	 ...
	 * @return string
	 */
	public static function includedProductsForm($pageUrl = '', $isSupplier = false) {
...
//	}
		...
		$morehtmlcenter= '<div class="included-products-global-search-container" >';
		...
		$morehtmlcenter.= '<input name="sall" value="'.dol_htmlentities($sall).'" id="search-all-form-input" class="included-products-global-search-input" placeholder="'.$langs->trans('Search').'" autocomplete="off">';
		...
		if($globalCountResult > 0){
			$output.= '<div class="includedproducts__results-count">';
...
//	else $moreforfilter = $hookmanager->resPrint;
		...
		$output.= '<table class="noborder centpercent included-products-results" >';
		$output.= '<thead>';

		$output.= '<tr class="included-products-row --search liste_titre">';

		$output.= '	<th class="included-products-col --ref" >';
		$output.= ' <input type="text" class="flat"  name="search_ref" value="'.dol_htmlentities($search_ref).'" placeholder="'.$langs->trans('SearchRef').'" />';
		$output.= '	</th>';

		$output.= '	<th class="included-products-col --label" >';
		...
		if($conf->stock->enabled){
			$output.= '	<th class="included-products-col --stock-reel center" ></th>';
			$output.= '	<th class="included-products-col --stock-theorique center" ></th>';
		}

		if ($conf->fournisseur->enabled) {
			$output .= '	<th class="included-products-col --buy-price" ></th>';
		}

		$output.= '	<th class="included-products-col --subprice" ></th>';
		$output.= '	<th class="included-products-col --discount" ></th>';
		$output.= '	<th class="included-products-col --finalsubprice" ></th>';
		$output.= '	<th class="included-products-col --qty" ></th>';

		if (!empty($conf->global->PRODUCT_USE_UNITS)) {
			$output.= '<th class="included-products-col --unit" >';
			$output.= '</th>';
		}

		$output.= '	<th class="included-products-col --finalprice" ></th>';
		$output.= '	<th class="included-products-col --action" >';
		...
		$classForSortLink = "included-products-sort-link";

		$output.= '<tr class="included-products-row --title liste_titre">';
		$output.= '	<th class="included-products-col --ref" >'
		...
		if($conf->stock->enabled){
			$output.= '	<th class="included-products-col --stock-reel center" >'.$langs->trans('RealStock').'</th>';
			$output.= '	<th class="included-products-col --stock-theorique center" >'.$langs->trans('VirtualStock').'</th>';
			$colnumber+=2;
		}

		if ($conf->fournisseur->enabled) {
			$colnumber++;
			$output .= '	<th class="included-products-col --buy-price" >' . ($isSupplier ? $langs->trans('PredefinedFournPricesForFill').img_help(1, $langs->trans('PredefinedFournPricesForFillHelp')) : $langs->trans('BuyPrice')) . '</th>';
		}
		$output.= '	<th class="included-products-col --subprice" >'.$langs->trans('Price').'</th>';
		$output.= '	<th class="included-products-col --discount" >'.$langs->trans('Discount').'</th>';
		$output.= '	<th class="included-products-col --finalsubprice" >'.$langs->trans('FinalDiscountSubPrice').'</th>';
		$output.= '	<th class="included-products-col --qty" >'.$langs->trans('Qty').'</th>';
		...
		$output.= '	<th class="included-products-col --finalprice" >'.$langs->trans('FinalDiscountPrice').'</th>';
		$output.= '	<th class="included-products-col --action" >';
			...
			if($curentCountResult > 0){
				while ($obj = $db->fetch_object($querySearchRes)){
					...
					if($resProd > 0){
						...
						$output.= '<tr class="included-products-row --data" data-product="'.$product->id.'"  >';
						$output.= '<td class="included-products-col --ref" >'. $product->getNomUrl(1).'</td>';
						$output.= '<td class="included-products-col --label" >'. self::highlightWordsOfSearchQuery($product->label, $search_label.' '.$sall).'</td>';
						if($conf->stock->enabled) {
							$output .= '<td class="included-products-col --stock-reel" >' . $product->stock_reel . '</td>';
							$output .= '<td class="included-products-col --stock-theorique" >' . $product->stock_theorique . '</td>';
						}

						if ($conf->fournisseur->enabled) {
							$output .= '<td class="included-products-col --buy-price" >';
...
//						$output.= '</div>';
						...
						//
						$output.= '<td class="included-products-col --subprice right nowraponall" >';
						$output.= '<input id="included-products-list-input-subprice-'.$product->id.'"  data-product="'.$product->id.'"   class="included-products-list-input-subprice right on-update-calc-prices" type="number" step="any" min="0" maxlength="8" size="3" value="'.$subprice.'" placeholder="x" name="prodsubprice['.$product->id.']" />';
						$output.= ' '.$langs->trans("HT");
						$output.= '</td>';

						// REDUCTION EN %
						$output.= '<td class="included-products-col --discount center" >';
						$output.= '<input id="included-products-list-input-reduction-'.$product->id.'"  data-product="'.$product->id.'"   class="included-products-list-input-reduction center on-update-calc-prices" type="number" step="any" min="0" max="100" maxlength="3" size="3" value="'.$reduction.'" placeholder="%" name="prodreduction['.$product->id.']" />';
						$output.= '%';
						$output.= '</td>';

						// FINAL SUBPRICE AFTER REDUCTION
						$output.= '<td class="included-products-col --finalsubprice right" >';
						...
						// QTY
						$output.= '<td class="included-products-col --qty" >';
						$qty = 1;
						$output.= '<input id="included-products-list-input-qty-'.$product->id.'"  data-product="'.$product->id.'"  class="included-products-list-input-qty center on-update-calc-prices" type="number" step="any" min="0" maxlength="8" size="3" value="'.$qty.'" placeholder="x" name="prodqty['.$product->id.']" />';
						$output.= '</td>';

						// UNITE
						if (!empty($conf->global->PRODUCT_USE_UNITS)) {
							$output.= '<td class="included-products-col --unit" >';
							$output.= $product->getLabelOfUnit();
							$output.= '</td>';
						}

						$output.= '<td class="included-products-col --finalprice right" >';
...
//					$output.= '</div>';
						...
						$output.= '<tr class="included-products-row">';
						$output.= '<td class="included-products-col-error center" colspan="'.$colnumber.'">'. $product->errorsToString() .'</td>';
						$output.= '</tr>';
				...
				$output.= '<tr class="included-products-row">';
				$output.= '<td class="included-products-col-no-result" colspan="'.$colnumber.'">'. $langs->trans("NoResults") .'</td>';
				$output.= '</tr>';
			...
			$output.= '<tr class="included-products-row">';
			$output.= '<td class="included-products-col-error" colspan="'.$colnumber.'">'. $db->error() .'</td>';
			$output.= '</tr>';
		}
