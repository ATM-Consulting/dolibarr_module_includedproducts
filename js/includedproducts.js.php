...
/**
 * \file    js/includedproducts.js.php
 * \ingroup includedproducts
 * \brief   JavaScript file for module includedproducts
 */
...
$langs->loadLangs(array("includedproducts@includedproducts","other"));
...
$confToJs = array(
	...
	'dec' => $dec,
	'thousand' => $thousand,
	'interface_url' => dol_buildpath('includedproducts/scripts/interface.php',1),
	'supplierElements' => $IncludedProducts->supplierElements
);
...
/* Javascript library of module includedproducts */
$( document ).ready(function() {
	...
	$(document).on("change", "[name^=prodfourprice]", function() {
		...
		if(IncludedProducts.isSupplierDocument()){
			let fk_product = $(this).attr("data-product");
			$("#included-products-list-input-subprice-" + fk_product).val($(this).find(':selected').data('up'));
			$("#included-products-list-input-subprice-" + fk_product).trigger('change');
		}
	});
	...
	$(document).on("change", ".on-update-calc-prices" , function(event) {
		let fk_product = $(this).attr("data-product");
		IncludedProducts.updateLinePricesCalcs(fk_product)
	});
	...
	$(document).on("click", ".included-products-list-action-btn.--addProductToLine" , function(event) {
		event.preventDefault();
		let fk_product = $(this).attr("data-product");
		IncludedProducts.addProductToCurentDocument(fk_product);
	});
	...
	$(document).on("change", ".included-products-list-input-qty" , function(event) {
		if(DiscountRule != undefined && !IncludedProducts.isSupplierDocument()){ // il faut bien vérifier que discountrule est présent
...
(function(o) {
	...
	o.disableAddProductFields = function (fk_product, disable = true){
		...
		setTimeout(function() {
			...
			$("#included-products-list-input-qty-"+fk_product).prop("disabled",disable);
			buttonAddProduct.prop("disabled",disable);
			$("#included-products-list-input-subprice-"+fk_product).prop("disabled",disable);
			$("#included-products-list-input-reduction-"+fk_product).prop("disabled",disable);
	...
	o.updateLinePricesCalcs = function (fk_product){
		...
		inputQty = $("#included-products-list-input-qty-"+fk_product);
		inputSubPrice = $("#included-products-list-input-subprice-"+fk_product);
		inputReduction = $("#included-products-list-input-reduction-"+fk_product);
	...
	o.addProductToCurentDocument = function (fk_product){
		...
		var sendData = {
			...
			'qty': $("#included-products-list-input-qty-"+fk_product).val(),
